<template>
  <div class="сomponents">
    ...
    <div class="сomponents__counter сomponents-counter" v-if="withCounter">
      <IconBtn class="сomponents-counter__btn сomponents-counter__btn--dec" :lock="counter === 1"
        icon="/img/icons/minus.svg" @click="dec()" />

      <div class="сomponents-counter__value">
        {{ counter }}
      </div>
      <IconBtn class="сomponents-counter__btn сomponents-counter__btn--inc" icon="/img/icons/plus.svg"
        @click="inc()" />
    </div>
      ...
      <li class="сomponents-list__item сomponents-list-item" v-for="item in components" :key="item.url">
        <NuxtLink class="сomponents-list-item__link сomponents-list-item-link" :to="`/${item.url}`">
          <picture class="сomponents-list-item-link__picture">
            ...
            <img class="сomponents-list-item-link__img" width="142" height="142" loading="lazy"
              :alt="`Зображення ${item.name}`" title="" />
          </picture>
