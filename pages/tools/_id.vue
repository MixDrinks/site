...
export default {
  ...
  async asyncData({ route, error, query, $axios }) {
    ...
    const cocktailsFullPromise = $axios.get(`/v2/filter/tools=${route.params.id}${page}`).catch(() => {
      return error({
        statusCode: 404,
        message: "This page could not be found",
      });
  ...
  methods: {
    async updateCocktails(payload) {
      ...
      const cocktails = await this.$axios.get(`/v2/filter/tools=${this.$nuxt.$route.params.id}${page}`);
